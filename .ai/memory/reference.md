...
- **External systems:**
  ...
  - Resend — email. `/api/subscribe` adds a contact to a segment; `/api/daily-reminder` sends a daily broadcast to that segment.
  - Google Fonts — Bricolage Grotesque, Archivo Black, Schibsted Grotesk.
  - Dictionary — `an-array-of-english-words` (bundled).
- **Secrets/env:** `RESEND_API_KEY`, `RESEND_SEGMENT_ID`, `CRON_SECRET`. Set in the Vercel project env; `.env.local` for local dev (git-ignored via `*.local`, auto-loaded in `bun run dev`). Never commit values.
